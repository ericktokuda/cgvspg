...
%   test image: data_seismic
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function deno_Seismic
clf;
% original image
im = imread('data_seismic.jpg','jpg');
im = double(im);
% create noisy image
Noisyim = GWNoisy2(im,7);
...
disp(' ');
tic;
im_wrec = TIDenoiseHard2(Noisyim,5,qmf,3*5);
toc;
psnr2 = 20*log10(255/(1/512*norm(double(im(:))-im_wrec(:))));
disp('take (extended) shearlet transform for denoising')
disp(' ');
imrec = HT_DST(Noisyim,qmf2,qmf2,1,[4 4 4 4 5],5,2,1,1,0.9*[4 4 4 4 4],'both');
