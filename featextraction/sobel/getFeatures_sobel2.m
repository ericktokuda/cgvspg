function features=getFeatures_sobel(pathToImage)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script to extract the features from an image,
%  based on Anderson ideas.
%  The process involves two steps: 
%  - extraction of maximum
%  - fitting to gaussians
%
% INPUT: full path of the RGB image
%   e.g. getFeatures_sobel('/mnt/ext3/Datasets512/CG/3dcommunity_01.jpg')
%
% OUTPUT: 150d-array
%
...
%disp ('I need to change it (I am still applying sobel twice)!');
...
sizeGauss = 5;  % Size of the window of Gaussian
...
%filt=fspecial('log'); %Choose one kind of filter
...
aux(aux==-1)=[];
features=aux';
if (length(features)<sizeFeatures ) %just colored images
    disp (['Skipping ', pathToImage, ': one colour band']);    
    features=-1;
    return;
end
...
res=sortedList(end-s+1:end,1:2); %Get the s greater elements
